...
def draw(canvas):
    ...
    collide = group_collide(rock_group, my_ship)
    if collide: 
        lives -= 1
        if lives == 0: 
            new_game()
...
def rock_spawner():
    ...
    # Limit the rock spawner
    if len(rock_group) <= 12:
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)

        # Dont spawn rocks near the ship
        distance = dist(my_ship.get_position(), a_rock.get_position())
        if distance > my_ship.get_radius() + a_rock.get_radius() + 10:
